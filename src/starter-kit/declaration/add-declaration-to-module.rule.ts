...
export function addDeclarationToAppModule(appModule: string, option: Schema, type: string): Rule {
 ...
 /*  console.log({module:res})
     console.log({'_options.path':option.path}) */
      ...
      const componentPath = option.path === 'src/app' || isFullCustom ? kebab(option.name) : ( !!option.dataName ? kebab(option.path).replace('src/app/', '') : (kebab(option.path).replace('src/app/', '') + '/' + kebab(option.name)) );
